      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            _AgentBanner(auth: auth),
            _QuickMenu(auth: auth),
            Padding(
...
class _ProfileHeader extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final name = auth.fullName.isEmpty ? (SupabaseService.currentUser?.email ?? 'Khách Hải Tín') : auth.fullName;
    final phone = auth.phone.isEmpty ? 'Chưa cập nhật số điện thoại' : auth.phone;
      ...
      child: Row(
        children: [
          const CircleAvatar(
            ...
            child: Icon(Icons.person_rounded, size: 36, color: AppTheme.goldColor),
          ),
          ...
          if (auth.isAgent)
            Container(
              ...
              child: const Text(
                'ĐẠI LÝ',
                style: TextStyle(color: AppTheme.charColor, fontWeight: FontWeight.w900, fontSize: 11),
              ),
            ),
        ],
      ),
...
class _AgentBanner extends StatelessWidget {
  const _AgentBanner({required this.auth});

  final AuthProvider auth;

  @override
  Widget build(BuildContext context) {
    if (auth.agentStatus == null) {
      return const SizedBox.shrink();
    }

    if (auth.agentStatus == 'approved') {
      return Container(
        ...
        child: Text(
          ...
          style: Theme.of(context).textTheme.bodySmall?.copyWith(color: AppTheme.goldColor),
        ),
      );
    }

    if (auth.agentStatus == 'pending') {
      return Container(
        width: double.infinity,
        margin: const EdgeInsets.fromLTRB(20, 0, 20, 10),
        padding: const EdgeInsets.all(12),
    ...
    return Container(
      ...
      child: Text(
        auth.rejectReason == null || auth.rejectReason!.isEmpty ? 'Đã từ chối khách sỉ' : 'Từ chối: ${auth.rejectReason}',
...
class _QuickMenu extends StatelessWidget {
  const _QuickMenu({required this.auth});
  ...
  Widget build(BuildContext context) {
    final items = [
      (Icons.badge_outlined, 'Thông tin cá nhân', () => _openAndRefresh(context, const PersonalInfoScreen())),
      (Icons.location_on_outlined, 'Địa chỉ giao hàng', () => _openAndRefresh(context, const ShippingAddressScreen())),
      (Icons.workspace_premium_outlined, 'Đăng ký làm Khách Sỉ', () => _openAndRefresh(context, const BecomeAgentScreen())),
        ...
        child: Column(
          children: List.generate(items.length, (index) {
            ...
            return Column(
              children: [
                ListTile(
                  leading: Icon(item.$1, color: AppTheme.goldColor),
                  title: Text(item.$2),
                  trailing: const Icon(Icons.chevron_right_rounded, color: AppTheme.mutedColor),
                  onTap: item.$3,
                ),
...
class _OrderCard extends StatelessWidget {
  ...
  String _money(dynamic value) {
    final amount = value is num ? value.round() : int.tryParse(value?.toString() ?? '') ?? 0;
  ...
  Widget build(BuildContext context) {
    ...
    final isHistory = status == 'Lịch sử mua' || status == 'Đã giao' || status == 'Hoàn tất';
  ...
  Future<void> _buyAgain(BuildContext context, List<Map<String, dynamic>> items) async {
    ...
    for (final item in items) {
      ...
      cartItems.add(
        CartItem(
          ...
          quantity: item['quantity'] is int ? item['quantity'] as int : int.tryParse(item['quantity']?.toString() ?? '') ?? 1,
          weight: (item['weight'] ?? (products[index].weights.isEmpty ? '500g' : products[index].weights.first)).toString(),
          grindType: (item['grind_type'] ?? (products[index].grindOptions.isEmpty ? 'Xay pha phin' : products[index].grindOptions.first)).toString(),
        ),
      );
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(cartItems.isEmpty ? 'Không tìm thấy sản phẩm để mua lại' : 'Đã thêm lại đơn hàng vào giỏ'),
      ),
    );
  }
}
...
void _showOrderDetail(BuildContext context, Map<String, dynamic> order) {
  showModalBottomSheet(
    ...
    builder: (context) => DraggableScrollableSheet(
      ...
      minChildSize: 0.5,
      maxChildSize: 0.92,
      builder: (context, controller) => _OrderDetailSheet(order: order, controller: controller),
    ),
  );
}
      ...
      children: [
        Center(
          child: Container(
            width: 42,
            height: 4,
            decoration: BoxDecoration(color: AppTheme.lineColor, borderRadius: BorderRadius.circular(999)),
          ),
        ),
        ...
        _DetailLine(label: 'Mã vận đơn', value: (order['tracking_code'] ?? 'Đang chờ tạo vận đơn').toString()),
        _DetailLine(label: 'Đơn vị vận chuyển', value: (order['carrier'] ?? 'Hải Tín Delivery').toString()),
        const SizedBox(height: 12),
        Stepper(
