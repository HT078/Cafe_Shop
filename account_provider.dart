...
class AccountProfile {
  ...
  factory AccountProfile.fromSupabase(
    ...
    final status = profile?['agent_status']?.toString().trim();
    final normalizedStatus = status == null || status.isEmpty ? null : status;
    ...
    return AccountProfile(
      ...
      rejectReason: profile?['reject_reason']?.toString(),
...
class AccountProvider extends ChangeNotifier {
    ...
    try {
      ...
      try {
        ...
        if (remoteAddresses.isNotEmpty) {
          _addresses = _normalizeDefault(remoteAddresses);
        }
    ...
    if (user != null) {
      try {
        await SupabaseService.updateProfileById(user.id, {
          ...
          'phone': businessPhone,
          'agent_note': note.isEmpty
              ? 'Sản lượng dự kiến: $expectedVolume'
              : 'Sản lượng dự kiến: $expectedVolume\n$note',
  ...
  void saveAddress(AddressItem address) {
